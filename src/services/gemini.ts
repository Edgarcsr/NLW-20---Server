...
export async function transcribeAudio(audioBase64: string, mimeType: string) {
  const res = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o áudio para prtugês Brasil, Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em parágrafos quando for apropriado',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  const prompt = `
  Com base no texto fornecido abaixo como contexto, responda à pergunta de forma cara e precisa em português do Brasil.
  ...
  if (!response.text) {
    throw new Error('Falha ao gerar pelo Gemini')
  }
